...
def saturation_specific_humidity(temp, months=True):
    '''
    ...
    if months==True:
        es = xr.DataArray(es, coords=[temp.months, temp.pfull, temp.lat, temp.lon],
                      dims=['months', 'pfull', 'lat', 'lon'])
    else:
        es = xr.DataArray(es, coords=[temp.time, temp.pfull, temp.lat, temp.lon],
                      dims=['time', 'pfull', 'lat', 'lon'])
        
    ws = 0.622 * es / (temp.pfull - es)  # Needs to check again??
    qs = ws / (1 + ws)
    if months==True:
        qs = xr.DataArray(qs, coords=[temp.months, temp.pfull, temp.lat, temp.lon],
                    dims=['months', 'pfull', 'lat', 'lon'])
    else:
